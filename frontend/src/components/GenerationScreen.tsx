...
import { Play, Sparkles, Loader2, X } from "lucide-react";
...
export default function GenerationScreen({
  ...
  // ✅ store interval globally so we can clear it safely
  ...
  const startProduction = async () => {
    ...
    try {
      ...
      intervalRef.current = window.setInterval(async () => {
        try {
          const sRes = await fetch(`${BACKEND_URL}/job-status/${job_id}`);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
            ...
            {gallery.length === 0 ? (
              ...
              gallery.map((vid, i) => (
                ...
                >
                  <video src={vid.url} className="rounded-lg w-full" />
