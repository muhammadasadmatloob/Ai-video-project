...
def build_timed_subtitles(words, actual_duration):
    ...
    # Chunk into max 3 words to keep captions fully on-screen
    chunks = []
    curr = []
    curr_len = 0

    for w in cleaned_words:
        # Max 3 words or 18 chars per caption chunk to prevent overflow
        if len(curr) >= 3 or (curr_len + len(w)) > 18:
            if curr:
    ...
    for i, c in enumerate(chunks):
        ...
        # If over 16 chars, split at first space for two-line display
        if len(formatted_word) > 16 and " " in formatted_word:
