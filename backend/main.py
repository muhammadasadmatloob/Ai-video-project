...
async def start_video_generation(
    ...
    background_tasks.add_task(run_full_pipeline, topic, job_id, user_id, ratio, duration)
...
async def run_full_pipeline(topic, job_id, user_id, ratio, duration):
    try:
        ...
        # 2. Asset Processing (Pass orientation to avoid duplicates correctly)
        orientation = "portrait" if ratio == "9:16" else "landscape"
        assets = await process_video_job(script, job_id, user_folder, duration, orientation)
        ...
        jobs[job_id] = {
            "status": "completed", 
            "url": f"http://127.0.0.1:8000/temp/{user_id}/{os.path.basename(final_video_path)}",
            "progress": 100
        }
...
async def get_user_gallery(user_id: str):
    user_folder = os.path.join(TEMP_DIR, user_id)
    if not os.path.exists(user_folder): return {"videos": []}
